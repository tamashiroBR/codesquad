...
import { EventTracker } from "../../server/squadEvents.mjs";
      ...
      if (s) {
        squads.push({
          ...
          agents: Array.isArray(s.agents)
            ? (s.agents as Record<string, unknown>[])
...
async function buildSnapshot(squadsDir: string, tracker: EventTracker): Promise<WsMessage> {
  const activeStates = await readActiveStates(squadsDir);
  // Seed/refresh the event tracker so a freshly connected client gets a baseline
  // timeline (idempotent: re-recording an unchanged state yields no new events).
  for (const [name, state] of Object.entries(activeStates)) {
    tracker.record(name, state);
  }
  return {
    type: "SNAPSHOT",
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          const snapshot = await buildSnapshot(squadsDir, tracker);
