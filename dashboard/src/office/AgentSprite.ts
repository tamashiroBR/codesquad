...
export class AgentSprite {
  ...
  constructor(
    ...
    // Pulsing status glow under the active/checkpoint agent — drawn relative to
    // the graphics origin so it can be tween-scaled around the agent's feet.
    this.statusGlow = scene.add.graphics();
    this.statusGlow.setPosition(x, y + 26);
    this.statusGlow.setDepth(y - 2);
  ...
  private drawStatusGlow(status: AgentStatus): void {
    this.glowTween?.stop();
    this.glowTween = undefined;
    this.statusGlow.clear();
    this.statusGlow.setScale(1);
    this.statusGlow.setAlpha(1);

    // Only highlight agents that are actively engaged.
    const highlight = status === 'working' || status === 'delivering' || status === 'checkpoint';
    if (!highlight) return;

    const color = STATUS_COLORS[status] ?? COLORS.statusWorking;
    this.statusGlow.fillStyle(color, 0.30);
    this.statusGlow.fillEllipse(0, 0, 120, 30);
    this.statusGlow.lineStyle(2, color, 0.55);
    this.statusGlow.strokeEllipse(0, 0, 120, 30);
    ...
    this.glowTween = this.scene.tweens.add({
      ...
      duration: status === 'checkpoint' ? 700 : 1100,
      ease: 'Sine.easeInOut',
      yoyo: true,
      repeat: -1,
    });
