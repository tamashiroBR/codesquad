...
1. You have already loaded:
   - The squad's `squad.yaml` (passed to you by the Codesquad skill)
   - The squad's `squad-party.csv` (all agent personas)
   - Company context from `_codesquad/_memory/company.md`
...
3. **Resolve skills**: Read `squad.yaml` → `skills` section. For each non-native skill (anything other than web_search, web_fetch):
   a. Verify `skills/{skill}/SKILL.md` exists
      ...
      - If yes → read `_codesquad/core/skills.engine.md`, follow Operation 2 (Install)
...
2. **Update squad memory** — write to BOTH files (runs after Post-Completion Cleanup above):
   ...
   **Technical routing:** For any technical learning (bugs, workarounds, API behavior):
   - If it affects any squad (Playwright bugs, OS rendering quirks, API limits) → write to the appropriate `_codesquad/core/best-practices/` file instead of `memories.md`
...
- If a step file is missing, inform the user and suggest running `/codesquad edit {squad}` to fix.
